  ...
  for(ii in 1:length(ttList)){
    ...
    for(jj in 1:nrow(ttList[[ii]])){
      ...
      if((is.na(ttList[[ii]][jj, pValIdx])) || (is.na(ttList[[ii]][jj, measIdx])) || (is.na(ttList[[ii]][jj, fcIdx]))){
        ...
        tt.p[ii, which(colnames(tt.p)==ttList[[ii]][jj, measIdx])] <- NA
        tt.lo[ii, which(colnames(tt.p)==ttList[[ii]][jj, measIdx])] <- NA
        tt.c[ii, which(colnames(tt.p)==ttList[[ii]][jj, measIdx])] <- NA
        tt.s[ii, jj] <- NA
        tt.fc[ii, which(colnames(tt.p)==ttList[[ii]][jj, measIdx])] <- NA
