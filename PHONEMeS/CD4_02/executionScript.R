...
load(file = "../build-Object/CD4_02/dataGMM_CD4_02.RData")
...
for(ii in 1:6){
  ...
  if(ii %in% 1:5){
    ...
    resultsMulti <- removeRedundantNodes(resultsSIF1 = resultsMulti)
    write.table(x = resultsMulti, file = paste0("CD4_cplex_exp_combined.txt"), quote = FALSE, sep = "\t", row.names = FALSE, col.names = TRUE)
