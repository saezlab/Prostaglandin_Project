...
for(i in 1:nrow(data.IDmap)){
  ...
  protein <- CD4$`Gene names`[i]
  site <- CD4$`Amino acid`[i]
  position <- CD4$Position[i]
...
##
# Obtaining information from each experimental condition separately
ep1Val <- CD4[, 6:10]
ep2Val <- CD4[, 11:15]
ep3Val <- CD4[, 16:20]
ep4Val <- CD4[, 21:25]
...
ep1Score <- log2(CD4$`Student's T-test q-value EP1_CT`/pValThresh)

ep2Score <- log2(CD4$`Student's T-test q-value EP2_CT`/pValThresh)

ep3Score <- log2(CD4$`Student's T-test q-value EP3_CT`/pValThresh)

ep4Score <- log2(CD4$`Student's T-test q-value EP4_CT`/pValThresh)

pgeScore <- log2(CD4$`Student's T-test q-value PGE2_CT`/pValThresh)

##
ep1.p <- CD4$`Student's T-test q-value EP1_CT`
ep1.lo <- ep1Score
...
##
ep2.p <- CD4$`Student's T-test q-value EP2_CT`
ep2.lo <- ep2Score
...
##
ep3.p <- CD4$`Student's T-test q-value EP3_CT`
ep3.lo <- ep3Score
...
##
ep4.p <- CD4$`Student's T-test q-value EP4_CT`
ep4.lo <- ep4Score
...
##
pge.p <- CD4$`Student's T-test q-value PGE2_CT`
pge.lo <- pgeScore
...
save(list=c("GMM.ID", "GMM","GMM.wFC"), file="dataGMM_CD4.RData")
