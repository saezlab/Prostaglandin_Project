...
readOutSIFAll<- function(cplexSolutionFileName, binaries = binaries){
  ...
  for(ii in 1:(length(cplexSolution)-1)){
    ...
    currSolution <- cplexSolution[[ii]]["variables"]
    
    if(length(currSolution)==1){
...
##
cascadeConstraints <- function(pknList = pknList, cascade = cascade, binaries = binaries){
  ...
  for(ii in 1:(length(cascade)-1)){
    ...
    for(i in 1:length(cascade[[ii]])){
      ...
      for(j in 1:length(cascade[[ii+1]])){
        ...
        # kk <- strsplit(x = kin, split = "_")[[1]][1]
        # ss <- strsplit(x = sbstr, split = "_")[[1]][1]
        
        idx <- intersect(x = which(pkn$K.ID==kin), y = which(pkn$S.ID==sbstr))
        
        for(jj in 1:length(idx)){
          
          if(jj == 1){
            
            cc <- binaries[[1]][which(binaries[[3]]==paste0("reaction ", pkn$K.ID[idx[jj]], "=", pkn$S.cc[idx[jj]]))]
            
          } else {
            
            cc <- paste0(cc, " + ", binaries[[1]][which(binaries[[3]]==paste0("reaction ", pkn$K.ID[idx[jj]], "=", pkn$S.cc[idx[jj]]))])
          }
