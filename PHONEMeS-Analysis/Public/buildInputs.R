  ...
  for(ii in 1:nrow(Table.ID)){
    ...
    for(jj in 1:length(tableTopList)){
      ...
      if(names(GMM)[[ii]]%in%rownames(tt)){
        ...
        #
        GMM[[ii]][jj, 1] <- as.character(log(x = tt[which(rownames(tt)==names(GMM)[ii]), idxPval]/pThresh, base = 2))
        GMM.wFC[[ii]][jj, 1] <- as.character(log(x = tt[which(rownames(tt)==names(GMM)[ii]), idxPval]/pThresh, base = 2))
        
        if(!is.na(log(x = tt$adj.P.Val[which(rownames(tt)==names(GMM)[ii])]/pThresh, base = 2))){
          
          #
          if(is.null(fcThresh)){
            if(tt[which(rownames(tt)==names(GMM)[ii]), idxPval] <= pThresh){
            ...
            if(abs(tt[which(rownames(tt)==names(GMM)[ii]), idxFC]) >= fcThresh){
